...
Além disso, com o intuito de encapsular a entrada do método **"Strategy"** ``MostraVotacao()``, foi criado a classe ``TextoImagem``. Como nesta funcionalidade é informado ao eleitor sobre a votação através de texto e/ou imagem, foi criada esta clase que encapsula esse tipo de dados.
